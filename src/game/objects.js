...
export function Bullet(tex, x, y, angle) {
    ...
    Bullet.prototype.update = function(delta) {
            ...
            this.graphic.rotation = Math.atan2(this.speedY, this.speedX);
...
export function Shotgun() {
    ...
    Shotgun.prototype.shoot = function() {
            var bulletOffset = 15 * utils.DEG2RAD;
            return this.state.add([this.spawnBullet(this.x, this.y, this.angle),
...
export function Bubble(armor, x, y, angle) {
    GameObject.call(this);
    this.x = Number(x);
    this.y = Number(y);
    armor = Number(armor);

    this.speedX = Math.cos(Number(angle) * utils.DEG2RAD);
    this.speedY = 1;
    ...
    if (_.isNumber(armor)) {
        ...
        utils.warn('Bubble armor is not a number');
    }
    ...
    Bubble.prototype.update = function(delta) {
        ...
        if (this.hp <= -1) {
            ...
            if (this.crack) {
                this.crack.gotoAndStop(Math.round((1 - (this.hp / this.hpMax)) * (this.crack.totalFrames-1)));
            }
        }
    ...
    Bubble.prototype.speedToggle = function(tempSpeed) {
        if (!_.isNumber(this._originalSpeed)) {
            ...
            utils.warn('Bubble.speedToggle: this._original speed was not set before attempting to toggle speed.\r'+
    ...
    Bubble.prototype._getElementMethod = function(type, element) {
        if (element) {
            var eleMethod = this['_' + type + utils.capitalize(element)];
    ...
    Bubble.prototype._updateFire = function() {
        if (this.fireConfig) {
            ...
            var chance = utils.randomRange(0, 100);
...
export function WarriorBubble(armor, x, y, angle) {
...
export function Particle(emitter, tex, opt) {
    this.x = opt.x;
    this.y = opt.y;

    this.speed = opt.speed;
    this.speedX = Math.cos(opt.angle * utils.DEG2RAD);
    this.speedY = -Math.sin(opt.angle * utils.DEG2RAD);
...
export function Emitter(tex, opt) {
    this.setOptions(opt);
    this.texture = tex;
    this.batch = new gfx.pixi.ParticleContainer();
            ...
            if (this.state) {
                for (var i=0; i<amount; ++i) {
                    var particle = new Particle(this, this.texture, {
                        x:        utils.randomRange(x - this.range, x + this.range),
                        y:        utils.randomRange(y - this.range, y + this.range),
                        speed:    utils.randomRange(this.speedMin, this.speedMax),
                        angle:    utils.randomRange(this.angleMin, this.angleMax),
                        lifeTime: utils.randomRange(this.lifeMin, this.lifeMax),
                        rotationRate: utils.randomRange(this.minRotationRate, this.maxRotationRate),
                    });
...
// types = oneshot, loop
export function Timer(duration, type, onComplete, onTick) {
...
export function FireEmber(bubble) {
    this.bubble = bubble;
    this.x = bubble.x;
    this.y = bubble.y;
    ...
    this.speedX = Math.cos(angle * utils.DEG2RAD);
    this.speedY = -Math.sin(angle * utils.DEG2RAD);
...
export function LightningShockwave(bubble) {
    this.bubble = bubble;
    this.x = bubble.x;
    this.y = bubble.y;
